...
async def eval(
    ...
    while completion_lock.locked():
        ...
        with completion_lock:
            ...
            # 根据概率分布随机选择停止标点数量：2个（40%），3个（30%），4个（20%），5个（10%）
            rand = random.random()
            if rand < 0.2:
                stop_symbol_count = 2
            elif rand < 0.5:  # 0.2 + 0.3 = 0.5
                stop_symbol_count = 3
            elif rand < 0.7:  # 0.5 + 0.2 = 0.7
                stop_symbol_count = 4
            else:  # 0.7 + 0.3 = 1.0
                stop_symbol_count = 5
            
            print(f"Randomly selected stop symbol count: {stop_symbol_count} (probability: {'40%' if stop_symbol_count == 2 else '30%' if stop_symbol_count == 3 else '20%' if stop_symbol_count == 4 else '10%'})")
            print(f"模型实际收到的提示词: {prompt}")
            
            try:
                # 保存初始状态，用于在达到停止条件时恢复
                initial_model_state = copy.deepcopy(model.model_state) if hasattr(model, 'model_state') else None
                initial_model_tokens = copy.deepcopy(model.model_tokens) if hasattr(model, 'model_tokens') else []
                ...
                for (
                    ...
                    if stop is not None:
                        ...
                        for stop_token in stop_list:
                            if stop_token in delta or stop_token in response:
                                print(f"Stop generation: found stop token {repr(stop_token)} in delta {repr(delta)}")
                                # 如果包含停止标记，只返回停止标记之前的内容
                                if stop_token in delta:
                                    delta = delta.split(stop_token)[0]
                                    if delta:
                                        if stream:
                                            yield json.dumps(
                                                {
                                    ...
                                    if initial_model_state is not None:
                                        model.model_state = initial_model_state
                                    if hasattr(model, 'model_tokens'):
                                        model.model_tokens = initial_model_tokens
                                    # 直接break，不执行后续的state更新
                    ...
                    print(f"Delta: {repr(delta)}, Current punctuation: {current_punctuation}, Total: {punctuation_count}")
